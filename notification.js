class ZenNotification {
    constructor() {
        ...
        this.notifications = []; // To keep track of active notifications
        ...
        if (!this.styleInjected) {
            ...
            style.textContent = `
				.zen-notification-container {
					...
					max-width: 80%; /* Prevent overflowing very small screens */
					/* Add a specific max-width for larger screens if needed */
					/* Example: max-width: 450px; */
				}
				...
				.zen-notification {
					...
					/* Try a fixed max-width for larger screens */
					width: 100%; /* Let content determine width */
					max-width: 400px; /* Maximum width of the notification itself */
				}
    ...
    notify(title, message, timeout = 5000, options = {}) {
        ...
        let autoTimeoutId;
        let resolvePromise; // To store the resolve function of the internal Promise
        ...
        this.createButtons(buttonContainer, buttons, notification, () => resolvePromise()); // Resolve on button click
        ...
        if (countdown) {
            ...
            this.startCountdown(countdownElement, timeout, countdownText, notification, () => resolvePromise()); // Resolve on countdown end
        } else {
            autoTimeoutId = setTimeout(() => {
                this.removeNotification(notification, () => resolvePromise()); // Resolve on timeout
            }, timeout);
        }
        ...
        const returnObject = {
            ...
            close: () => {
                ...
                this.removeNotification(notification, () => resolvePromise()); // Resolve on manual remove
            },
            onClose: async () => {
    ...
    getIconClass(type) {
        switch (type) {
            ...
            default: return 'fa-info-circle'; // More generic default icon
        }
    }
    ...
    createButtons(container, buttonsConfig, notificationElement, resolve) {
        buttonsConfig.forEach(button => {
            ...
            if (button.onClick) {
                ...
                btn.addEventListener('click', () => this.removeNotification(notificationElement, resolve)); // Auto-close if no onClick and not explicitly disabled
            }
    ...
    recalculatePositions() {
        // With flex-direction: column on the container, the browser handles the vertical stacking.
        // This function is kept for potential future layout changes (e.g., stacking from top).
        const notifications = this.container.querySelectorAll('.zen-notification');
        let topPosition = 20;
        notifications.forEach(notification => {
            notification.style.top = `${topPosition}px`;
            topPosition += notification.offsetHeight + 10; // Adjust spacing as needed
        });
    }
}
